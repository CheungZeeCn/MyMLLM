...
class CustomTrainingArguments(Seq2SeqTrainingArguments):
    output_dir: Optional[str] = field(
        default="/home/ana/data4/output_models/MyMLLM/100k_p2s_wiki_pretrain_2tasks_debug",
...
def collator(batch, max_length=1024):
    ...
    if batch[0]['prompt'] != '':
        ...
        # 这里要非常注意，如果prompt长度不一，会有很多pad，在预训练的效果验证阶段，不合适讲eval batch size 设置为 >1 ;
...
if __name__ == '__main__':
    ...
    training_args.output_dir = "/home/ana/data4/output_models/MyMLLM/100k_p2s_wiki_pretrain_2tasks_debug"

    train_dataset, eval_dataset, test_dataset = None, None, None
    train_dataset = wiki_pretrain_dataset(data_args, os.path.dirname(data_args.train_file), processor, 'train', max_records=100, load_cache=True)
    logging.info(f"train_dataset size: {len(train_dataset)}")
    eval_dataset = wiki_pretrain_dataset(data_args, os.path.dirname(data_args.validation_file), processor, 'eval', max_records=10, load_cache=True)
