...
model_path = "/home/ana/data4/output_models/MyMLLM/100k_p2s_prerain_wo_ocr_lvocab/checkpoint-6400"
...
def collator(batch, max_length=1024):
    ...
    if batch[0]['prompt'] != '':
        ...
        prompts_inputs = processor(text=prompts, padding=False, return_tensors="pt", add_special_tokens=False,
                                max_length=max_length, truncation=True)
...
if __name__ == '__main__':
    ...
    train_dataset = wiki_warmup_dataset(data_args, os.path.dirname(data_args.train_file), processor, 'train', prompt='按顺序识别图片中的文字: ')
    logging.info(f"train_dataset size: {len(train_dataset)}")
    eval_dataset = wiki_warmup_dataset(data_args, os.path.dirname(data_args.validation_file), processor, 'eval', max_records=10, prompt='按顺序识别图片中的文字: ')
