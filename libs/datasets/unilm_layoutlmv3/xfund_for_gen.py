"""
    将xfund 的ner任务 转化为 文本生成任务，仅仅输出 title, 也就是这里面的 HEADER;
"""
    ...
    def load_data(
        ...
        for i in range(len(data_file['documents'])):
            ...
            for j in range(len(data_file['documents'][i]['document'])):
                ...
                if cur_item['label'].upper() == "HEADER":
                    cur_doc_text_labels.append(cur_item['text'])
            cur_doc_text_labels = "#".join(cur_doc_text_labels)
            ...
            total_data['text_labels'].append(cur_doc_text_labels)
        ...
        for i in range(len(total_data['lines'])):
            ...
            cur_doc_text_labels = \
            self.decoder_tokenizer(total_data['text_labels'][i], truncation='longest_first', add_special_tokens=False,
